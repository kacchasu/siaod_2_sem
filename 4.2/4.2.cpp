...
void delete_auto(SheetAuto& sheet, string rnumber)
{
    ...
    for (int i = tmp; i < sheet.n; i++)
        sheet.autosheet[i - 1] = sheet.autosheet[i];
    memcpy(sheet.autosheet, sheet.autosheet, sheet.n - 1);
    sheet.n--;
}
...
int main()
{
    ...
    for (int i = 0; i < endn; i++)
    {
        ...
        cin >> sheetauto.autosheet[amount - 1];
        sheetauto.n++;
    }
    cout << sheetauto;
